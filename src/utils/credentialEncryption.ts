...
export class CredentialEncryption {
    ...
    private static getEncryptionKey(): Buffer {
        ...
        if (!key) {
            console.warn('⚠️  CREDENTIAL_ENCRYPTION_KEY not set. Using default key (NOT SECURE FOR PRODUCTION)');
    ...
    public static decrypt(encryptedToken: string): string {
        ...
        try {
            ...
            throw new Error('Failed to decrypt Discord token. Token may be corrupted or encryption key changed.');
        }
    }
    ...
    public static validateDiscordToken(token: string): boolean {
        ...
        const tokenRegex = /^[A-Za-z0-9+/]{24}\.[A-Za-z0-9+/]{6}\.[A-Za-z0-9+/=-]{27,}$/;
