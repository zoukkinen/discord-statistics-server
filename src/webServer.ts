...
export class WebServer {
  ...
  private setupRoutes(): void {
    ...
    this.app.get("/api/stats", async (req, res) => {
      try {
        const eventId = req.query.eventId
          ...
          : undefined;
        const stats = await this.database.getCurrentStats(eventId);
        res.json(stats);
      } catch (error) {
    ...
    this.app.get("/api/stats/history/members", async (req, res) => {
      try {
        ...
        // If no date range provided, use a default range (e.g., last 7 days)
        if (!start || !end) {
          const now = new Date();
    ...
    this.app.get("/api/stats/history/games", async (req, res) => {
      try {
        let start = req.query.start as string;
        let end = req.query.end as string;
        const eventId = req.query.eventId
    ...
    this.app.get("/api/stats/top-games", async (req, res) => {
      try {
    ...
    this.app.get("/api/recent-activity", async (req, res) => {
      try {
        ...
        const limitedActivity =
          limit && limit > 0 ? activity.slice(-limit) : activity;
      ...
      async (req, res) => {
        try {
          ...
          if (discordToken || discordGuildId) {
            ...
            if (
              discordToken &&
              !CredentialEncryption.validateDiscordToken(discordToken)
            ) {
              return res
                .status(400)
                .json({ error: "Invalid Discord token format" });
            }

            if (
              discordGuildId &&
              !CredentialEncryption.validateDiscordGuildId(discordGuildId)
            ) {
              return res
                .status(400)
                .json({ error: "Invalid Discord Guild ID format" });
            }
          }
          ...
          // Add Discord credentials if provided
          if (discordToken && discordGuildId) {
            eventData.discordToken = discordToken;
            eventData.discordGuildId = discordGuildId;
          }
      ...
      async (req, res) => {
        try {
          ...
          if (!activeEvent) {
            return res
              .status(404)
              .json({ error: "Event not found after activation" });
          }
        ...
        if (activeEvent) {
          ...
          const eventConfig = {
            ...
            description:
              ...
              `Discord activity tracking for ${transformedEvent.name}`,
          };
...
/**
 * Create and return an Express app instance for testing
...
export async function createServer(): Promise<any> {
  ...
  return webServer["app"]; // Access private app property for testing
}
