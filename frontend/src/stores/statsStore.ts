...
const fetchMemberHistory = async (): Promise<MemberStats[]> => {
  ...
  if (
    ...
    configStore.config.startDate === "2025-07-31T00:00:00+03:00"
  ) {
  ...
  if (config?.startDate && config?.endDate) {
    ...
    console.log(
      "fetchMemberHistory: Using config dates:",
    ...
    console.log(
      "fetchMemberHistory: Using fallback dates:",
      startDate.toISOString(),
      "to",
      endDate.toISOString()
    );
...
export const statsStore = {
  ...
  async fetchMemberHistory() {
    try {
      ...
      console.log(
        "memberHistory signal updated, new length:",
  ...
  get totalActivePlayers() {
    return currentStats().currentGames.reduce(
      (sum, game) => sum + game.player_count,
      0
    );
  },
