...
const Charts: Component<ChartsProps> = (props) => {
  ...
  const createChart = async () => {
    ...
    try {
      ...
      console.log(
        "Creating chart with Assembly Summer 2025 data:",
      ...
      if (memberHistory.length > 0) {
        console.log(
          "Date range:",
          new Date(memberHistory[0].timestamp).toLocaleString(),
          "to",
          new Date(
            memberHistory[memberHistory.length - 1].timestamp
          ).toLocaleString()
        );
        console.log(
          "Member count range:",
          Math.min(...memberHistory.map((p) => p.online_members)),
          "to",
          Math.max(...memberHistory.map((p) => p.online_members)),
          "online members"
        );
      }
        ...
        <>
          ...
          <div class="chart-footer">
            <div class="chart-hint">
              🎮 {props.eventName || configStore.config.name} Discord Activity (
              {statsStore.memberHistory.length} data points)
            </div>
          </div>
        </>
      ) : (
